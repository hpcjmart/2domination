...
int loadfile_matrixMultiply(int argc, char **argv, int block_size, dim3 &dimsA, dim3 &dimsB, char *filename1, int iteraciones)
{
    ...
    for(unsigned int it=0;it<iteraciones;it++){
        ...
        matrixTropCUDA<32><<< grid, threads >>>(d_C, d_A, d_B, dimsA.x, dimsB.x); 
...
int main(int argc, char **argv)
{
    ...
    int recurrencia = busqueda_recurrencia(block_size,dimsA, dimsB, iteraciones);
    int recurrencia_minima = busqueda_recurrencia_minima(block_size,dimsA, dimsB);
